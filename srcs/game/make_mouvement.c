...
/*   Updated: 2024/11/21 17:31:32 by edubois-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	make_action(t_data *data, unsigned int old_nb_moove)
{
	if ((data->pressed[SDL_SCANCODE_W] || data->pressed[SDL_SCANCODE_UP])
		&& !(data->pressed[SDL_SCANCODE_S]
			|| data->pressed[SDL_SCANCODE_DOWN]))
		up_pressed(data);
	if ((data->pressed[SDL_SCANCODE_A] || data->pressed[SDL_SCANCODE_LEFT])
		&& !(data->pressed[SDL_SCANCODE_D]
			|| data->pressed[SDL_SCANCODE_RIGHT]))
		left_pressed(data);
	if ((data->pressed[SDL_SCANCODE_S] || data->pressed[SDL_SCANCODE_DOWN])
		&& !(data->pressed[SDL_SCANCODE_W]
			|| data->pressed[SDL_SCANCODE_UP]))
		down_pressed(data);
	if ((data->pressed[SDL_SCANCODE_D] || data->pressed[SDL_SCANCODE_RIGHT])
		&& !(data->pressed[SDL_SCANCODE_A]
			|| data->pressed[SDL_SCANCODE_LEFT]))
		right_pressed(data);
