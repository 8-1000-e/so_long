...
/*   Updated: 2024/11/21 17:37:05 by edubois-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (y < data->max_y)
		{
			if (data->map[x][y] == '1')
				mlx_put_image_to_window(data->mlx, data->win, data->walls,
					y * 48, x * 48);
			if (data->map[x][y] == 'C')
				mlx_put_image_to_window(data->mlx, data->win, data->items,
					y * 48, x * 48);
			y++;
		}
		x++;
...
void	put_rhino(t_data *data)
{
	...
	mlx_put_image_to_window(data->mlx, data->win, data->rhino,
		random_y * 48, random_x * 48);
	data->r.x = random_x;
	data->r.y = random_y;
}
